...
\paragraph{Setup.}
Let $E_f$ be the output of the DGF algorithm on the complete graph on $P$.\\
Let $E = \{(p,q) \in E_f \| \delta_2(p,q) \leq t \cdot |pq|\}$, 
which means, 
all the edges that fail the $t-leapfrog$ property.\\
...
Initialize $E^* \gets E$.\\
Sort $E^*$ in non ascending order.\\
For every $(p,q) \in E^*$:\\
$\; F \gets F \, \cup \, T_{(pq)} $\\
$\; E^* \gets E^* \, \setminus \, T_{(pq)} $\\
...
whose total weight is at most $\wt(\MST(P))$; combined with the
